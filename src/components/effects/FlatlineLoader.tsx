...
export default function FlatlineLoader({ 
 ...
 >
 {/* ECG Monitor frame */}
 <div className="relative w-[400px] h-[200px] bg-gray-900 border-2 border-gray-700 p-4 shadow-2xl">
 {/* Monitor bezel */}
 <div className="absolute top-2 left-4 flex items-center gap-2">
 <div className={`w-2 h-2 ${phase === 'flatline' ? 'bg-zinc-700' : 'bg-zinc-600'} animate-pulse`} />
 ...
 <stop offset="20%"stopColor={phase === 'flatline' ? '#3d3d3d' : '#4a4a4a'} />
 <stop offset="80%"stopColor={phase === 'flatline' ? '#3d3d3d' : '#4a4a4a'} />
 <stop offset="100%"stopColor="transparent"/>
 </linearGradient>
 <filter id="ecgGlow">
 <feGaussianBlur stdDeviation="2"result="blur"/>
 <feFlood floodColor={phase === 'flatline' ? '#3d3d3d' : '#4a4a4a'} floodOpacity="0.8"/>
 ...
 <g opacity="0.1">
 {[...Array(6)].map((_, i) => (
 <line key={`h${i}`} x1="0"y1={i * 20} x2="300"y2={i * 20} stroke="#4a4a4a"strokeWidth="0.5"/>
 ))}
 {[...Array(16)].map((_, i) => (
 <line key={`v${i}`} x1={i * 20} y1="0"x2={i * 20} y2="100"stroke="#4a4a4a"strokeWidth="0.5"/>
 ))}
 </g>
 ...
 phase === 'flatline' ? 'text-zinc-500' : 
 phase === 'charging' ? 'text-zinc-400' :
 ...
 <div className="mt-4 w-[300px] h-2 bg-gray-800 overflow-hidden">
 <motion.div
 className={`h-full ${phase === 'flatline' ? 'bg-zinc-700' : 'bg-zinc-600'}`}

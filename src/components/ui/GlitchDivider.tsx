...
export default function GlitchDivider({ 
 ...
 {/* Scanning dot */}
 <motion.div
 className="absolute top-1/2 -translate-y-1/2 w-2 h-2 bg-zinc-700"
 style={{
 ...
 {['01001011', 'â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆ', 'FF00AA', 'â–“â–’â–‘', 'CORRUPT'].map((text, i) => (
 <motion.span
 key={i}
 className="absolute font-mono text-[8px] text-zinc-500/30"
 style={{ left: `${15 + i * 18}%` }}
 animate={{
 opacity: [0, 0.3, 0],
 y: [0, -5, 0],
 ...
 <motion.span
 className="font-mono text-xs text-zinc-500/70 tracking-[0.3em]"
 animate={{
 opacity: [0.5, 1, 0.5],
 }}
 transition={{
 ...
 <div className="absolute top-0 left-0 w-2 h-px bg-zinc-700"/>
 <div className="absolute top-0 left-0 h-2 w-px bg-zinc-700"/>
 </div>
 <div className="absolute right-0 top-1/2 -translate-y-1/2 w-4 h-4">
 <div className="absolute top-0 right-0 w-2 h-px bg-zinc-700"/>
 <div className="absolute top-0 right-0 h-2 w-px bg-zinc-700"/>
 </div>
 </div>
 );
}

...
const typeColors = {
 ...
 report: 'text-blood-500 bg-blood-500/10',
};

const statusColors = {
 draft: 'text-zinc-400 bg-zinc-800',
 review: 'text-zinc-400 bg-zinc-400/10',
 pending: 'text-blood-500 bg-blood-500/10',
};
...
function DraftCard({ 
 ...
 {draft.autoSaved && (
 <span className="text-blood-500">Auto-saved</span>
 )}
 </div>
...
export default function DraftsPage() {
 ...
 { label: 'In Review', value: drafts.filter(d => d.status === 'review').length, color: 'text-zinc-400' },
 { label: 'Pending', value: drafts.filter(d => d.status === 'pending').length, color: 'text-blood-500' },
 { label: 'Auto-saved', value: drafts.filter(d => d.autoSaved).length, color: 'text-blood-500' },
 ].map((stat, i) => (

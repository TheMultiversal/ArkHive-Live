...
// This is a template page - actual data will be fetched based on slug
...
export default function InvestigationDetailPage() {
  ...
  // Get investigation data from database or use default
  const investigationData = investigationDatabase[slug] || defaultInvestigationData;

  // If no data found and not using default template, return 404
        ...
        <div className="lg:flex lg:gap-8">
          ...
          <article className="flex-1 max-w-4xl">
            ...
            <nav className="flex items-center gap-2 text-sm text-dark-400 mb-6">
              ...
              <ChevronRight className="w-4 h-4" />
              <span className="text-dark-300">{investigationData.category}</span>
            </nav>

            {/* Header */}
            <header className="mb-8">
              <div className="flex flex-wrap items-center gap-3 mb-4">
                <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border ${severityColors[investigationData.severity]}`}>
                  <AlertTriangle className="w-3 h-3" />
                  {investigationData.severity.toUpperCase()}
                </span>
                <span className="px-3 py-1.5 bg-dark-800 text-dark-300 text-xs font-medium rounded-full border border-dark-700">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <button className="flex items-center gap-2 px-4 py-2 bg-dark-800 hover:bg-dark-700 border border-dark-700 text-white text-sm rounded-lg transition-colors">
                ...
                <button 
                  ...
                  className="lg:hidden flex items-center gap-2 px-4 py-2 bg-blood-700 hover:bg-blood-600 text-white text-sm rounded-lg transition-colors"
                >
            ...
            <div className="prose prose-invert prose-lg max-w-none mb-12">
              {investigationData.content.map((paragraph, index) => (
                <p key={index} className="text-dark-300 leading-relaxed mb-6">
                  {paragraph}
                </p>
              ))}
            </div>
              ...
              <div className="space-y-3">
                {investigationData.sources.map((source, index) => (
                  <a
                    ...
                    className="flex items-center justify-between p-3 bg-dark-800/50 hover:bg-dark-800 rounded-lg transition-colors group"
                  >
                    <div className="flex items-center gap-3">
                      <FileText className="w-5 h-5 text-dark-400" />
                      <div>
                        <span className="text-white group-hover:text-blood-500 transition-colors">
                          {source.title}
                        </span>
                        <span className="block text-xs text-dark-500">{source.type}</span>
                      </div>
                    </div>
                    <ExternalLink className="w-4 h-4 text-dark-400 group-hover:text-blood-500" />
                  </a>
                ))}
              </div>
            </div>

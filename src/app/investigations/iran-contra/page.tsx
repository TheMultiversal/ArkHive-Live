...
export default function IranContraPage() {
 ...
 <blockquote className="p-4 bg-[rgba(0,20,55,0.40)] border-l-4 border-blood-600 text-sm text-zinc-300 italic mb-4">
 {investigation.bolandAmendment.text}
 </blockquote>
 <p className="text-xs text-zinc-400">{investigation.bolandAmendment.passage}</p>
 <p className="text-sm text-blood-400 mt-2">{investigation.bolandAmendment.violation}</p>
 </motion.div>

 {/* ICJ Ruling */}
 <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="border-2 border-blood-800/60 bg-blood-950/20 p-6 mb-8">
 <h2 className="text-xl font-bold glass-text uppercase tracking-wider mb-4 flex items-center gap-2">
 ...
 <p className="text-sm text-blood-400 font-bold">{investigation.icjRuling.ruling}</p>
 ...
 <div key={idx} className="p-4 bg-blood-950/20 border border-blood-500/30">
 <p className="font-bold text-blood-400 text-sm">{crime.crime}</p>
 <p className="text-sm text-zinc-300 mt-2">{crime.description}</p>
 <p className="text-xs text-blood-500 font-bold mt-2">{crime.toll}</p>
 </div>
 ))}
 </div>
 </motion.div>

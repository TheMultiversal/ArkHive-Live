...
export default function RussianInterferencePage() {
 ...
 <span className="text-xs text-zinc-300">{op.type}</span>
 </div>
 ...
 key={idx}
 href={attendee.href}
 className="border border-[rgba(255,255,255,0.18)] bg-[#0a0a0a] p-3 hover:border-zinc-700 transition-colors group"
 >
 <h3 className="font-bold glass-text group-hover:text-white">{attendee.name}</h3>
 <p className="text-sm text-zinc-400">{attendee.role}</p>
 </Link>
 ) : (
 <div key={idx} className="border border-[rgba(255,255,255,0.18)] bg-[#0a0a0a] p-3">
 ...
 <span className="text-xs text-zinc-400">Evidence: {obs.evidence}</span>
 </div>
 ...
 <span className={`text-xs ${conv.pardoned ? 'text-zinc-400' : 'text-zinc-400'}`}>

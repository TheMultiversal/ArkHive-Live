...
export default function PoliceBrutalityPage() {
 ...
 <h2 className="text-xl font-bold glass-text uppercase tracking-wider mb-4 flex items-center gap-2"><BarChart3 className="w-5 h-5 text-blood-500"/>Scale of Violence</h2>
 ...
 <p className="text-xs text-blood-400 font-bold">{k.outcome}</p>
 </div>
 ))}
 </div>
 </motion.div>
 ...
 <h2 className="text-xl font-bold glass-text uppercase tracking-wider mb-4 flex items-center gap-2"><Shield className="w-5 h-5 text-blood-500"/>Qualified Immunity: License to Kill</h2>
 ...
 <h3 className="text-sm font-bold text-blood-400 uppercase tracking-wider mb-2">Absurd Examples</h3>
 <ul className="space-y-2">
 {investigation.qualifiedImmunity.absurdExamples.map((ex, idx) => (
 <li key={idx} className="text-sm text-zinc-300 pl-4 border-l-2 border-blood-500/50 py-1">{ex}</li>
 ))}
 </ul>
 </motion.div>
 <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="glass-card p-6 mb-8 border-l-4 border-blood-600">

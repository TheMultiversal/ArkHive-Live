...
export default function DominionDefamationPage() {
 ...
 <div key={idx} className={`p-4 border ${item.type === 'Settlement' ? 'border-blood-500/30 bg-blood-950/30' : item.type === 'Judgment' ? 'border-blood-500/30 bg-blood-950/30' : 'border-[rgba(60,160,255,0.12)] bg-zinc-900/30'}`}>
 ...
 <li key={idx} className="text-sm text-zinc-400 flex items-start gap-2 p-2 bg-zinc-900/30 border border-blood-500/20">
 <span className="text-blood-500 font-bold">✗</span>
 <span className="line-through opacity-70">{claim}</span>
 </li>
 ))}
 </ul>
 <p className="text-xs text-zinc-500 mt-4 p-2 bg-blood-900/20 border border-blood-500/20">
 ...
 <div className="p-3 bg-[rgba(0,20,55,0.40)] border border-blood-500/30">

...
const getTypeColor = (type: ActivityItem['type']) => {
 ...
 connection: 'text-blood-500 bg-blood-500/10',
...
export default function ActivityPage() {
 ...
 { label: 'Today', value: mockActivities.filter(a => isToday(a.timestamp)).length, icon: <Clock className="w-5 h-5 text-blood-500"/> },
 { label: 'Active Users', value: new Set(mockActivities.map(a => a.user.id)).size, icon: <User className="w-5 h-5 text-blood-500"/> },
 { label: 'Alerts', value: mockActivities.filter(a => a.type === 'alert').length, icon: <AlertTriangle className="w-5 h-5 text-blood-500"/> },
 ].map((stat, i) => (

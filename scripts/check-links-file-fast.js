...
const REQ_TIMEOUT = parseInt(process.argv[4], 10) || 6000;
...
async function checkUrl(url) {
  try {
    ...
    catch (e) { /* HEAD failed - will try GET */ }
  ...
  async function worker() {
    while (idx < urls.length) {
      ...
      try {
        ...
        results[i] = { url, ...(res), occurrences: map.get(url) };
      } catch (err) {
        results[i] = { url, error: String(err), occurrences: map.get(url) };
      }
    }
  }

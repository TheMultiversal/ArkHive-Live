...
// Agencies [slug] page
const agencyPage = `'use client';

import { useParams } from 'next/navigation';
import { motion } from 'framer-motion';
import Link from 'next/link';
import { ArrowLeft, AlertTriangle, Landmark, Shield } from 'lucide-react';
...
export default function AgencyDetailPage() {
    ...
    return (
      <div className="min-h-screen pt-20 lg:pt-24 pb-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center py-20">
          ...
          <h1 className="text-3xl font-black text-white uppercase tracking-wider mb-4">Agency Not Found</h1>
          <p className="text-zinc-400 mb-8">The agency could not be located in the database.</p>
  ...
  const relatedInvestigations = Object.entries(investigationDatabase).filter(([, inv]) =>
    inv.affiliations?.some((a) => a.href === \`/entities/agencies/\${slug}\`)
  );

  return (
    <div className="min-h-screen pt-20 lg:pt-24 pb-16">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <Link href="/entities/agencies" className="inline-flex items-center gap-2 text-zinc-400 hover:text-white transition-colors mb-6 pt-4">
          <ArrowLeft className="w-4 h-4" />
          Back to Agencies
        </Link>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="border-2 border-zinc-800/60 bg-[rgba(0,6,20,0.90)] p-6 lg:p-8 mb-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className={\`px-3 py-1 text-xs font-bold uppercase border \${riskColors[agency.riskLevel] || riskColors.moderate}\`}>
...
// Corporations [slug] page
const corpPage = `'use client';
...
export default function CorporationDetailPage() {
  const params = useParams();
  const slug = typeof params.slug === 'string' ? params.slug : '';
  const corp = corporationData[slug];
  ...
  if (!corp) {
    return (
      <div className="min-h-screen pt-20 lg:pt-24 pb-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center py-20">
          <AlertTriangle className="w-16 h-16 text-zinc-600 mx-auto mb-6" />
          <h1 className="text-3xl font-black text-white uppercase tracking-wider mb-4">Corporation Not Found</h1>
          <p className="text-zinc-400 mb-8">The corporation could not be located in the database.</p>
          <Link href="/entities/corporations" className="inline-flex items-center gap-2 px-6 py-3 bg-zinc-900 border border-zinc-700 text-zinc-300 hover:text-white hover:border-zinc-500 transition-all">
            <ArrowLeft className="w-4 h-4" />
  ...
  const relatedInvestigations = Object.entries(investigationDatabase).filter(([, inv]) =>
    inv.affiliations?.some((a) => a.href === \`/entities/corporations/\${slug}\`)
  );

  return (
    <div className="min-h-screen pt-20 lg:pt-24 pb-16">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <Link href="/entities/corporations" className="inline-flex items-center gap-2 text-zinc-400 hover:text-white transition-colors mb-6 pt-4">
          <ArrowLeft className="w-4 h-4" />
          Back to Corporations
        </Link>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="border-2 border-zinc-800/60 bg-[rgba(0,6,20,0.90)] p-6 lg:p-8 mb-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className={\`px-3 py-1 text-xs font-bold uppercase border \${riskColors[corp.riskLevel] || riskColors.moderate}\`}>
              {corp.riskLevel} risk
            </span>
            <span className="px-3 py-1 text-xs font-bold uppercase border border-zinc-700 bg-zinc-900 text-zinc-400">
              {corp.role}
          ...
          <h1 className="text-3xl lg:text-4xl font-black text-white uppercase tracking-wider mb-4">
            <GlitchText text={corp.name} />
          </h1>
          <p className="text-zinc-400 leading-relaxed">{corp.description}</p>
        </motion.div>
...
// Write all pages
const pages = [
  { dir: path.join(base, 'agencies', '[slug]'), file: 'page.tsx', content: agencyPage },
  { dir: path.join(base, 'corporations', '[slug]'), file: 'page.tsx', content: corpPage },
  { dir: path.join(base, 'organizations', '[slug]'), file: 'page.tsx', content: orgPage },
];

for (const p of pages) {
  const filePath = path.join(p.dir, p.file);
  fs.writeFileSync(filePath, p.content, 'utf8');
  console.log(`Wrote ${filePath} (${p.content.split('\n').length} lines)`);
}

// Also fix corrupted AgencyContent.tsx and CorporationContent.tsx  
// Just make them re-export the main page component since the page.tsx now handles everything
const agencyContentFix = `// This file is deprecated - agency detail rendering is now in page.tsx
export {};
`;
const corpContentFix = `// This file is deprecated - corporation detail rendering is now in page.tsx
export {};
`;
fs.writeFileSync(path.join(base, 'agencies', '[slug]', 'AgencyContent.tsx'), agencyContentFix, 'utf8');
fs.writeFileSync(path.join(base, 'corporations', '[slug]', 'CorporationContent.tsx'), corpContentFix, 'utf8');
console.log('Fixed AgencyContent.tsx and CorporationContent.tsx');

console.log('\nAll entity pages fixed!');

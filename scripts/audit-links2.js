...
for (const d of fs.readdirSync(invDir)) {
  if (d !== '[slug]' && fs.statSync(path.join(invDir, d)).isDirectory()) {
    validSlugs.investigations.add(d);
  }
}
...
function walk(dir) {
  ...
  for (const f of fs.readdirSync(dir)) {
    ...
    if (fs.statSync(fp).isDirectory()) {
      if (!['node_modules', '.next', '.next-old', '.git', 'scripts'].includes(f)) {
...
for (const file of files) {
  ...
  const hrefRe = /(?:href|to)[:=]\s*['"`]\/entities\/(individuals|agencies|corporations|organizations)\/([\w-]+)/g;
  while ((m = hrefRe.exec(content))) {
    if (!validSlugs[m[1]].has(m[2])) {
      broken.push({ file: file.replace(/\\/g, '/'), type: m[1], slug: m[2] });
    }
  }
  
  const invRe = /(?:href|to)[:=]\s*['"`]\/investigations\/([\w-]+)/g;
  while ((m = invRe.exec(content))) {
    if (!validSlugs.investigations.has(m[1])) {
      broken.push({ file: file.replace(/\\/g, '/'), type: 'investigation', slug: m[1] });
    }
  }
}
...
for (const [type, items] of Object.entries(byType)) {
  const u = [...new Set(items.map(i => i.slug))];
  console.log('\n' + type + ' (' + u.length + ' unique missing):');
